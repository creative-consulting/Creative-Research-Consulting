...
if (!process.env.GMAIL_USER) {
  console.error('GMAIL_USER is not set in environment variables');
}

if (!process.env.GMAIL_PASS) {
  console.error('GMAIL_PASS (App Password) is not set in environment variables');
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const emailHtml = `
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            <div class="field"><span class="label">Name:</span><span>${name}</span></div>
            <div class="field"><span class="label">Email:</span><span>${email}</span></div>
            <div class="field"><span class="label">Phone:</span><span>${phone}</span></div>
            <div class="field"><span class="label">Location:</span><span>${location}</span></div>
            <div class="field"><span class="label">Field of Interest:</span><span>${fieldOfInterest.join(', ')}</span></div>
            ${message ? `<div class="field"><span class="label">Message:</span><span>${message}</span></div>` : ''}
            ${file && file.size > 0 ? `<div class="field"><span class="label">Attachment:</span><div class="attachment">${file.name} (${Math.round(file.size / 1024)} KB)</div></div>` : ''}
          </div>
        </div>
      </body>
      </html>
    ...
    // Nodemailer transporter (Gmail Workspace)
    ...
    return NextResponse.json(
      { message: 'Application submitted successfully!', messageId: info.messageId },
      { status: 200 }
    );
