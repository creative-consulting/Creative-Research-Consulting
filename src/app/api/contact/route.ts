...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const transporter = nodemailer.createTransport({
      service: 'gmail',
      auth: {
        user: process.env.GMAIL_USER, // your Gmail/Workspace email
        pass: process.env.GMAIL_PASS  // your App Password (not real password)
      }
    });

    const mailOptions = {
      from: formData.email || process.env.GMAIL_USER,
      to: process.env.TO_EMAIL_ADDRESS!,
      subject: `New Contact Form: ${name}`,
      text: emailText,
      html: emailHtml
